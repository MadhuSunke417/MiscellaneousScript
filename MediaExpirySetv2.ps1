...
#Create object TSEnvironment COMObject
...
If ($TSEnv.Value("_SMSTSMEDIAEXPIRE")){
    ...
    #Write-Host "P1 is $p1"
    #Write-Host "P2 is $p2"
    $Medate = $MediaExpiryDate 
    
    #string to date coverstion
    $Medate = [datetime]::parseexact($Medate, 'MM-dd-yyyy', $null)
    $currentdateBIOS = Get-Date
    $daysleft = ($Medate - $currentdateBIOS).Days
    ...
    $WarningParams = @{
    Title = "Media Expiry Info"
    TitleFontSize = 20
    TitleBackground = 'Orange'
    TitleTextForeground = 'Black'
    Content = "***WARNING*** This Media expiry date set to $Medate and days left $daysleft"
}
...
$ErrorParams = @{
    ...
    TitleBackground = 'Red'
    TitleTextForeground = 'Black'
    Content = "***ERROR*** This Media expired!!!"
}

if($Valid){
    New-WPFMessageBox @WarningParams 
   }
else{
    New-WPFMessageBox @ErrorParams  
   }
...
Else {
    $Medate = "" 
    #continue without showing popup
}

#set value custom var SMSTSMediaExpiryDate to expiry date
$TSEnv.Value("SMSTSMediaExpiryDate") = $Medate
# Stop logging
Stop-Transcript
}catch{
    Write-Output "Running Outside of TS env."
    }
